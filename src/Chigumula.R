...
Chigumula<-microbiology |> 
  filter(farm == "Chigumula") 

# Step 1: Calculate Mean Inhibition and Standard Error
summary_data <- Chigumula |> 
...
line_plot <- ggplot(summary_data, aes(x = item, y = mean_inhibition, group = 1)) +
  ...
  labs(title = "Microbial Inhibition Across Sample Types (Chigumula)",
...
#I want to do anova 
# Two way ANOVA
model <- aov(inhibition ~ antibiotic * item, data = Chigumula)
#tidy anova results
